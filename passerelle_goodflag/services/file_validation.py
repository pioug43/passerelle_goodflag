"""
Service de validation et parsing de fichiers.

Regroupe :
- Détection de type MIME par magic bytes
- Protection SSRF sur les URL de fichiers
- Validation structurelle PDF / DOCX
- Décodage base64 sécurisé (→ GoodflagValidationError)
- Extraction du fichier depuis les multiples sources du payload
"""
...
def safe_b64decode(data, field_name='file'):
    """
    Décode une chaîne base64 en bytes.

    Utilise le mode strict (``validate=True``) pour rejeter les données
    contenant des caractères non-base64 (espaces, retours à la ligne, etc.).
    Lève ``GoodflagValidationError`` au lieu de laisser remonter un
    ``binascii.Error`` brut (erreur 500).

    Si les données contiennent des espaces ou retours chariot légitimes
    (ex: base64 sur plusieurs lignes), elles sont nettoyées avant décodage.
    """
    if isinstance(data, str):
        # Nettoyer les espaces / retours chariot ajoutés par certains encodeurs
        data = data.strip().replace('\n', '').replace('\r', '').replace(' ', '')
    try:
        return base64.b64decode(data, validate=True)
    except (binascii.Error, ValueError) as exc:
        raise GoodflagValidationError(
            f"Contenu base64 invalide pour '{field_name}' : {exc}"
        ) from exc
...
def validate_file_content(content, content_type):
    ...
    - DOCX : signature ZIP (PK\\x03\\x04), fichier word/document.xml présent
    - Taille non nulle
...
# -- Extraction fichier depuis payload ---------------------------------


def parse_file_from_payload(payload, request, get_param, requests_session):
    """
    ...
    Args:
        ...
        get_param: callable(payload, key, default=None) pour extraire un paramètre.
        requests_session: session HTTP signée Passerelle (``self.requests``).
    ...
    filename = get_param(payload, 'filename')
    content_type = get_param(payload, 'content_type', 'application/pdf')
    file_content = None
    file_url = None
    ...
    if isinstance(file_obj, dict):
        ...
        file_content = safe_b64decode(file_b64, field_name='file.content')
    ...
    elif get_param(payload, 'file_base64'):
        file_b64 = get_param(payload, 'file_base64')
        if len(file_b64) > MAX_B64_LEN:
            raise GoodflagValidationError("File content exceeds maximum allowed size (50 MB)")
        file_content = safe_b64decode(file_b64, field_name='file_base64')
    elif get_param(payload, 'file_url'):
        file_url = get_param(payload, 'file_url')
        validate_file_url(file_url)
        resp = requests_session.get(file_url)
        ...
        for field_val in payload['fields'].values():
            if isinstance(field_val, dict) and 'content' in field_val:
                file_content = safe_b64decode(field_val['content'], field_name='fields.content')
